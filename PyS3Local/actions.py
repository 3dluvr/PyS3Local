...
def list_buckets(handler):
    ...
    xml = xml_templates.buckets_xml.format(buckets=xml).encode('utf-8')
...
def ls_bucket(handler, bucket_name, qs):
    ...
    if bucket:
        ...
        xml = xml_templates.bucket_query_xml.format(bucket_query=bucket_query, contents=contents).encode('utf-8')
        handler.wfile.write(xml)
    else:
        handler.send_response(404)
        handler.send_header('Content-Type', 'application/xml')
        handler.end_headers()
        xml = xml_templates.error_no_such_bucket_xml.format(name=bucket_name).encode('utf-8')
        handler.wfile.write(xml)
...
def get_acl(handler):
    ...
    handler.send_header('Content-Type', 'application/xml')
    handler.end_headers()
    handler.wfile.write(xml_templates.acl_xml.encode('utf-8'))
...
def delete_items(handler, bucket_name, keys):
    ...
    xml = xml_templates.deleted_xml.format(contents=xml).encode('utf-8')
    handler.wfile.write(xml)
